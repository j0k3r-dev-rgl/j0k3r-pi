...
Native rendering keeps `src/pi-runtime.ts` as a stable adapter, but the adapter imports the official `@earendil-works/pi-coding-agent` and `@earendil-works/pi-tui` package specifiers directly. API Tools owns their package contract through `peerDependencies` plus matching package-local `devDependencies` for tests/typecheck. The adapter must not use `createRequire`, custom search paths, sibling extension directories, filesystem probes, or fallback resolution.

Current source inspection confirms the replacement tools, continuation manager, and renderer are already wired through `registerApiTools`; this design phase does not re-open those completed implementation areas. The remaining dependency-contract mismatch is concrete: `src/pi-runtime.ts` includes `../../context7/` in `searchPaths` and probes candidate `node_modules` files, which violates active `REQ-TOOL-013 R2`/`SCN-TOOL-018` despite `render.ts` importing only the local adapter.
...
- Metadata/product constraints influencing design: hybrid OpenSpec authority, interactive phase authorization, strict TDD, exactly two independently enabled replacement tools, lossless bounded model output, Pi-owned rendering expansion, no arbitrary dependency, API Tools-owned official Pi runtime packages only through their official package contract, session-bound continuation, configured REST origin/base-path containment, and security-driven `api_status` URL-field removal.
- Metadata/PRD/spec conflicts_detected: None in the target design. The current `src/pi-runtime.ts` implementation is stale against `REQ-TOOL-013 R2` because its search path includes the sibling `context7` extension; `DES-DEP-001 R1` defines the required remediation. The remediated `REQ-TOOL-011` and `REQ-STS-001` through `REQ-STS-003` explicitly authorize URL-field removal while preserving safe non-URL status behavior.
...
| `DES-DEP-001` | `R1` | active | None | None | Keep `src/pi-runtime.ts` as a stable adapter with direct bare imports from `@earendil-works/pi-coding-agent` and `@earendil-works/pi-tui`; declare both as API Tools-owned `peerDependencies` and matching package-local `devDependencies`, and update the lockfile. | Retain `createRequire` plus custom search paths; borrow `context7/node_modules`; install arbitrary renderer packages; duplicate Pi rendering primitives locally. | This is the official Pi package contract described by project extension conventions, gives API Tools an independent test/typecheck environment, and satisfies `REQ-TOOL-013 R2`/`SCN-TOOL-018` without sibling-extension coupling. |
...
| Dependencies | Allow only API Tools-owned official Pi runtime packages through direct package specifiers; declare `@earendil-works/pi-coding-agent` and `@earendil-works/pi-tui` as peers with matching package-local development installs. Prohibit sibling paths, custom resolver search paths, and arbitrary additions. | Static review confirms only the two authorized package declarations/lock entries, `src/pi-runtime.ts` has direct imports and no `createRequire`/filesystem fallback, package-local tests/typecheck pass, and removing sibling extension installations does not affect resolution. |
...
| `extensions/api-tools/src/pi-runtime.ts` | Create/Modify | Stable adapter that directly imports and re-exports authorized Pi `keyHint` and `Text`; remove custom/sibling package resolution. |
| `extensions/api-tools/package.json` | Modify | Add only the authorized Pi runtime packages as `peerDependencies` and matching exact package-local `devDependencies`. |
| `extensions/api-tools/package-lock.json` | Modify | Record the API Tools-owned development installation for the two authorized Pi packages and their official transitive graph. |
...
- Relevant symbols/interfaces added or refined: `ApiIntegrationConfig`, `ApiIntegrationState`, `ContinuationManager`, `finalizeApiToolResult`, hardened endpoint resolver/redirect executor, `executeSwaggerAction`, `executeGraphqlAction`, `renderApiToolResult`, and the `src/pi-runtime.ts` `keyHint`/`Text` adapter exports are planned or confirmed names; exact implementation naming may follow local conventions without changing contracts.
- Validation map updates: package has only `npm test` and `npm run typecheck`; there is no separate package validation script. Add focused ownership for Swagger/GraphQL suites, static dependency/status-path checks, and a package-contract assertion that rejects `createRequire`, sibling-extension paths, custom `searchPaths`, and undeclared Pi imports.
- Conflicts or stale entries: the mapped `examples/extensions/truncated-tool.ts` does not exist; current renderer references that hard-code `ctrl+o` are unsuitable, while `extensions/context7/src/render/index.ts` remains a behavior reference only. The current API Tools `src/pi-runtime.ts` sibling search path is stale and must be replaced under `DES-DEP-001 R1`. The prior `api_status` conflict is resolved by active decision `DES-STS-001 R1`.
...
|-------|-------------|----------|
| Baseline | Existing retained tools and the prior apply's 41-test historical surface | Run fresh `npm test` and `npm run typecheck` before dependency remediation; record any pre-existing failure rather than treating prior apply evidence as current. |
...
| Static/package | Only the two authorized Pi packages are added as API Tools peers/dev installs; adapter uses direct imports with no sibling paths, `createRequire`, filesystem probes, or custom resolution; no legacy names, public `artifact_path`, or hard-coded expand key | Extend `render.test.ts` or the closest package-contract owner to inspect `src/pi-runtime.ts`, `package.json`, and lockfile after GREEN; run tests/typecheck from `extensions/api-tools`. |
...
Strict TDD ownership: adapt the existing `render.test.ts` package-boundary coverage for the dependency remediation; do not create a parallel one-case suite. The established Vitest framework and test root remain unchanged.
...
- [x] **Resolved and incorporated as `DES-DEP-001 R1`:** API Tools owns the two official Pi runtime package declarations and local development installation; `src/pi-runtime.ts` remains a direct-import adapter and performs no sibling/custom resolution.
