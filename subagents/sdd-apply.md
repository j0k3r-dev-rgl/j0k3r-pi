...
- **Durable Documentation Gate**: When field 4 (**Governing contracts and ready artifacts**) or ready contracts cite durable lifecycle documentation under `docs/` or an Implementation Readiness Packet, read those governing docs alongside the SDD contract. Ensure implementation satisfies both local SDD acceptance criteria and durable lifecycle requirements/ADRs. Record in `apply.md` under each `MINI-###` or `TASK-###` item: `- Canonical sources verified: <docs/path.md#STABLE-ID, or "None — change-local contract">`.
- Read only exact assigned `SKILL.md` paths. Do not inventory or scan `skills/`.
- **UI/UX gate**: When any approved task modifies frontend UI files (templates, components, pages, styles, layouts), check whether `impeccable-guide` is among the assigned skills or available in the project. If it is, read it and follow its **Subagent Prerequisite Gate** and autonomous command workflow for every UI change. If PRODUCT.md is missing, return `BLOCKED` immediately — do not attempt `impeccable init` or `impeccable shape` (these require user interaction that subagents cannot perform). If DESIGN.md is missing and code exists, run `impeccable document` autonomously. Once prerequisites are met, use only autonomous commands (`distill`, `layout`, `adapt`, `clarify`, `typeset`, `colorize`, `bolder`, `quieter`, `harden`, `onboard`, `polish`, `extract`, `audit`) following the execution order in the guide. Record in `apply.md` under each applicable `MINI-###` or `TASK-###` evidence record: `- Impeccable: <commands used in order, or "Not applicable — no UI changes">`. If impeccable is available but was not assigned, note `Impeccable: Available but not assigned` and proceed without it.
