---
name: tdd
description: "guide language-agnostic test-driven changes with existing-test discovery, framework reuse, mirrored test-tree organization, regression coverage, and red-green-refactor evidence."
license: Apache-2.0
metadata:
  author: j0k3r
  version: "1.0"
...
Use this skill for behavior-changing code work, bug fixes, regression fixes, refactors with behavior risk, or explicit requests to add, update, organize, or review tests. Apply it regardless of programming language or test framework.
...
- Define the expected behavior before changing code.
- RED: add or adapt the narrowest relevant test, run it, and confirm that it fails for the expected behavioral reason.
...
- GREEN: implement the smallest production change that makes the focused test pass, then run it again.
...
- Do not alter a valid expectation merely to make the implementation pass.
...
- Run the narrowest useful test during RED and GREEN, then run the relevant broader suite when practical.
- Preserve concise evidence for the selected test file, framework, RED failure reason, GREEN result, and post-refactor validation.
...
- existing tests encode behavior that conflicts with the requested expectation;
- RED cannot be demonstrated because the issue is not reproducible, the environment is unavailable, or required fixtures/dependencies are missing;
...
1. Confirm the expected behavior and affected production surface.
...
4. Select the closest semantically responsible test file; if none qualifies, justify a new file under the dedicated mirrored test tree.
5. Resolve any framework, colocation, migration, oversized-file, or conflicting-expectation decision gate with the user.
6. RED: adapt or create the approved test and confirm the expected failure with the narrowest relevant command.
7. GREEN: make the smallest implementation change and confirm the focused test passes.
8. REFACTOR only when useful, keeping responsibilities cohesive and removing duplication; rerun validation.
9. Run the relevant broader suite when practical and review changed test paths for mirrored organization and accidental duplication.
10. Report evidence, decisions, risks, and any validation that could not run.
...
- Existing test candidates inspected and the selected file, or the reason a new file was necessary.
- Detected framework and repository evidence supporting its use.
- Test location and how it mirrors the production source structure; include any user decision about colocated tests.
- RED command/result and expected failure reason.
- GREEN command/result and post-refactor or broader validation.
- Duplication/size review, remaining risks, blocked checks, and decisions still needed.
