...
- Validation or verification evidence must link requirement and acceptance IDs when it claims implementation or conformance validation. workflow verification is promoted only when it informs a named durable product, learning, implementation-conformance, quality, or release decision: link the exact `verify.md` requirement row, change-local workflow ID, and canonical requirement/acceptance IDs; store only the decision-relevant summary and provenance, never copy the full verification artifact. Ordinary technical verification remains in the workflow tree. Pre-requirement validation may instead trace to opportunity, assumption, hypothesis, journey, capability, or outcome IDs.
