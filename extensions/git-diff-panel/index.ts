...
async function openGitDiffPanel(ctx: ExtensionContext): Promise<void> {
	if (ctx.mode !== "tui") {
	...
	await ctx.ui.custom<undefined>(
		(tui, theme, _keybindings, done) => new GitDiffPanel(ctx.cwd, tui, theme, done),
		{
			overlay: true,
			overlayOptions: {
				width: "96%",
...
export default function gitDiffPanelExtension(pi: ExtensionAPI): void {
	pi.registerCommand("git-diff", {
		description: "Open a split git changes panel with tree and diff",
		handler: async (_args, ctx) => openGitDiffPanel(ctx),
	});
