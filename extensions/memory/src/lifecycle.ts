...
function buildMemoryInstructions(context: any): string {
  ...
  return [
    ...
    '- A canonical project profile is available through memory_project_profile. It is intentionally excluded from the regular startup memory slots. For substantial tasks in a project, call memory_project_profile with action=get early when relevant, and use action=update when durable project facts change.',
