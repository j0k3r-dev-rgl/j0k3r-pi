...
export async function generateTypeScriptSymbolCorpus(rootDir: string, size: 'medium' | 'large') {
  ...
  for (let index = 0; index < fileCount; index += 1) {
    ...
    await writeFile(join(rootDir, `file-${index}.ts`), `${source}\n`, 'utf8');
  }
  return { fileCount, symbolCount: fileCount * symbolsPerFile };
}

export async function generateJavaSymbolCorpus(rootDir: string, size: 'medium' | 'large') {
  const fileCount = size === 'large' ? 10_000 : 1_000;
