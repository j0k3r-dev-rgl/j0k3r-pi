...
The table-driven source is `test/fixtures/typescript-symbol-cases.ts`; parity assertions are in `test/typescript-symbol-parity.test.ts`. Fixture IDs, public fallback enums, schema/model versions, grammar versions, and README links are checked for documentation consistency.
...
Unsupported cases are explicit and do not authorize graph-only omission: an unproven, stale, partial, corrupt, or incompatible graph falls back to canonical direct analysis with structured diagnostics.
