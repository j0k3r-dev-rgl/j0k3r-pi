...
- Code Research registers its tools only when `graph.enable` is explicitly `true`.
- With graph enabled, refresh scheduling runs only on `session_start` reasons `startup`, `reload`, and `new`, plus supported source-file changes observed by the session watcher.
- Tool execution is read-only with respect to graph generation: tools do not build, refresh, or schedule refreshes.
...
- Older graph artifacts are treated as incompatible and regenerated by lifecycle/file-change refreshes instead of migrated.
- Tools do not use direct source inspection for graph queries. If the graph is missing, stale, partial, errored, incompatible, refreshing, unreadable, language-unsupported, or insufficient, the tool returns graph diagnostics.
...
- Code Research registra sus tools solo cuando `graph.enable` está explícitamente en `true`.
- Con graph habilitado, la sincronización corre solo en `session_start` con reasons `startup`, `reload` y `new`, más cambios observados en archivos fuente soportados.
- Ejecutar una tool no construye, refresca ni schedulea refresh del graph.
...
- Los artifacts viejos se consideran incompatibles y se regeneran por lifecycle/cambio de archivo, no desde queries.
- Si no, devuelven diagnósticos de graph unavailable; nunca inspección directa.
