...
import { buildGoProjectIndex, extractCalls, resolveGoCall, type GoCallable, type GoExtractedCall, type GoProjectIndex } from './workspace-graph.js';

interface ReverseEdge { caller: GoCallable; call: GoExtractedCall; receiverType?: string }

export async function executeGoReverseFunctionCallTree(
  cwd: string,
...
> {
  ...
  const root = buildNode(reverseEdges, candidates[0], input.max_depth ?? 10, 0, new Set());
...
function buildReverseEdges(index: GoProjectIndex): Map<string, ReverseEdge[]> {
  ...
  for (const caller of index.callables) {
    for (const call of extractCalls(caller.node)) {
      ...
      const list = reverse.get(resolved.callable.symbolId) ?? [];
      list.push({ caller, call, receiverType: resolved.receiverType });
      reverse.set(resolved.callable.symbolId, list);
    }
  }
  return reverse;
}

function buildNode(reverseEdges: Map<string, ReverseEdge[]>, callable: GoCallable, maxDepth: number, depth: number, visited: Set<string>): CallTreeNode {
  ...
  const callers = (reverseEdges.get(callable.symbolId) ?? []).map((edge) => {
    const callerNode = buildNode(reverseEdges, edge.caller, maxDepth, depth + 1, visited);
    callerNode.called_as = edge.call.text;
    callerNode.receiver_name = edge.call.receiver;
    callerNode.receiver_type = edge.receiverType;
    callerNode.call_line = edge.call.line;
    callerNode.call_column = edge.call.column;
    return callerNode;
  });
  if (callers.length > 0) node.callers = callers;
  return node;
}
