...
export function registerFunctionCallTreeTool(pi: any) {
  pi.registerTool({
    ...
    parameters: Type.Object({
      path: Type.String({
        description: 'Java file containing the root method, or directory to scan for the application index.',
      }),
      symbol: Type.String({
      ...
      kind: Type.Optional(
        Type.Union(
          [Type.Literal('method'), Type.Literal('function'), Type.Literal('class')],
          { description: 'Optional kind filter for the root symbol.' }
        )
      ),
      max_depth: Type.Optional(
        Type.Number({
          description: 'Maximum recursion depth for application-internal calls. Default: 10.',
        })
      ),
    ...
    async execute(_toolCallId: any, params: any, _signal: any, _onUpdate: any, ctx: any) {
      ...
      if (execution.status === 'not_found') {
        return {
          content: [{ type: 'text', text: execution.message }],
      ...
      return {
        content: [
          {
            type: 'text',
            text: `Call tree for ${execution.rootClassName}.${input.symbol}:\n\n${JSON.stringify(execution.result, null, 2)}`,
          },
        ],
